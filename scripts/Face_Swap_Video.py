...
def FrameCapture(path):
    ...
    fps = vidObj.get(5)
    output = cv.VideoWriter('Data/Data1OutputTRI.mp4',cv.VideoWriter_fourcc(*'MJPG'),30,frame_size)
    # Used as counter variable
    count = 0
    # checks whether frames were extracted
    success = 1
    while success:
        success, img2 = vidObj.read()
        img = cv.imread('Data/random.jpg')
        ...
        for face in faces:
            ...
            for n in range(68):
                ...
                #cv.circle(fake,(x,y),3,(0,0,3*n),-1)
        ...
        #Face 2
        for face in faces2:
            landmarks2 = predictor(im2gray, face)
            landmarks_points2 = []
            for n in range(68):
                ...
                #cv.circle(fake2,(x,y),3,(0,0,3*n),-1)
            ...
            for i in range(b2.shape[0]):
                cv.line(img2, (int(b2[i][0]),int(b2[i][1])), (int(b2[i][2]),int(b2[i][3])), linecolor, 1)
                cv.line(img2, (int(b2[i][4]),int(b2[i][5])), (int(b2[i][2]),int(b2[i][3])), linecolor, 1)
                cv.line(img2, (int(b2[i][0]),int(b2[i][1])), (int(b2[i][4]),int(b2[i][5])), linecolor, 1)
        ...
        ''''''''
        #showing triangles using function
        def show_triangle(tr1_index):
            cv.line(img3,points[tr1_index[0]], points[tr1_index[1]], (255,255,0), 2)
            cv.line(img3,points[tr1_index[1]], points[tr1_index[2]], (255,255,0), 2)
            cv.line(img3,points[tr1_index[0]], points[tr1_index[2]], (255,255,0), 2)
            #cv.imshow('function check', img3)
            
            cv.line(img4,points2[tr1_index[0]], points2[tr1_index[1]], (255,255,0), 1)
            cv.line(img4,points2[tr1_index[1]], points2[tr1_index[2]], (255,255,0), 1)
            cv.line(img4,points2[tr1_index[0]], points2[tr1_index[2]], (255,255,0), 1)
            #cv.imshow('correspondance check', img4)
            return
        ...
        #cv.imwrite("frame%d.jpg" % count, img)
        count += 1
